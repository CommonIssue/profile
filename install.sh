...
then
  ...
  do
    ...
    if [ "$INPUT" = "y" ]
    then
      echo "noverwriting current "~/ccminer/config.json"n"
      rm ~/ccminer/config.json
    elif [ "$INPUT" = "n" ]
    then
      echo "saving as "~/ccminer/config.json.#""
    else
      echo 'Invalid input. Please answer with "y" or "n".n'
    fi
  done
fi
...
#run the miner
screen -S CCminer -X stuff "~/ccminer/ccminer -c ~/ccminer/config.jsonn" 1>/dev/null 2>&1
printf 'nMining started.n'
printf '===============n'
printf 'nManual:n'
printf 'start: ~/.ccminer/start.shn'
printf 'stop: screen -X -S CCminer quitn'
printf 'nmonitor mining: screen -x CCminern'
printf "exit monitor: 'CTRL-a' followed by 'd'nn"
EOF
chmod +x start.sh
